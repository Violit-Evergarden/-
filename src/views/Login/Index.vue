...
const {$message,$router} = getCurrentInstance().appContext.config.globalProperties
const username = ref('')
const password = ref('')
...
async function loginClick(){
  ...
  if(status===1) return $message.error(message)
  if(status===0) {
    username.value = ''
    password.value = ''
